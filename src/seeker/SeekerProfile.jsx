...
const SeekerProfile = ({ user, appliedCount, savedCount, onLogout, onEdit }) => (
  <View style={{ padding: 20 }}>
    ...
    <Card style={{ alignItems: 'center', padding: 28, marginBottom: 20 }}>
      <Avatar url={getAvatarUrl(user.avatarUrl)} initials={user.avatar} size={72} color={COLORS.accent} />
      ...
      <Text style={{ fontSize: 14, color: COLORS.accent, fontFamily: FONTS.bodyMedium, marginTop: 4 }}>{user.title}</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 32, marginTop: 18 }}>
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontSize: 22, fontFamily: FONTS.headingBold, color: COLORS.accent }}>{appliedCount}</Text>
...
{(() => {
          ...
          return resumeUrl ? (
            <TouchableOpacity
              ...
              style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: COLORS.accent + '22', borderRadius: 6 }}>
              <Text style={{ fontSize: 11, color: COLORS.accent, fontFamily: FONTS.bodySemiBold }}>VIEW</Text>
            </TouchableOpacity>
          ) : null;
        })()}
      </View>
