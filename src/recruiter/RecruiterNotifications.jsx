...
const RecruiterNotifications = ({ notifications, onMarkAllRead }) => {
        ...
        sorted.map(notif => (
          <Card key={notif.id} style={{ marginBottom: 12, paddingLeft: 0, overflow: 'hidden' }}>
            <View style={{ flexDirection: 'row' }}>
              <View style={{ width: 3, borderTopLeftRadius: 12, borderBottomLeftRadius: 12, backgroundColor: notif.read ? COLORS.border : accent }} />
              <View style={{ flex: 1, padding: 14, flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
                <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: accent + '22', alignItems: 'center', justifyContent: 'center' }}>
                  <UserIcon size={18} color={accent} />
                </View>
                <View style={{ flex: 1 }}>
                ...
                {!notif.read && (
                  <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: accent, marginTop: 5 }} />
                )}
              </View>
            </View>
          </Card>
