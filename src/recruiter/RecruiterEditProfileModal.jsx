...
const RecruiterEditProfileModal = ({ user, onClose, onUpdated }) => {
    ...
    <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <KeyboardAvoidingView style={{ flex: 1, backgroundColor: COLORS.bg }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView style={{ flex: 1, padding: 20 }} showsVerticalScrollIndicator={false}>
          <View style={{ alignItems: 'center', marginBottom: 24 }}>
            <TouchableOpacity onPress={pickImage} style={{ position: 'relative' }}>
              <Avatar url={form.avatarUrl?.uri ? form.avatarUrl.uri : getAvatarUrl(form.avatarUrl)} initials={user.avatar} size={80} color={COLORS.recruiterAccent} />
              <View style={{ position: 'absolute', bottom: 0, right: 0, backgroundColor: COLORS.recruiterAccent, padding: 6, borderRadius: 12 }}>
          ...
          <Input label="Company Name" value={form.company} onChangeText={set('company')} />
          ...
          <Input label="Company Size" value={form.companySize} onChangeText={set('companySize')} />
          <Input label="Website" value={form.website} onChangeText={set('website')} />
          <Input label="Bio" value={form.bio} onChangeText={set('bio')} multiline numberOfLines={3} />
          
          <View style={{ marginTop: 10, marginBottom: 20 }}>
            <Text style={{ fontSize: 13, color: COLORS.textSecondary, fontFamily: FONTS.body, marginBottom: 8 }}>Resume / Document</Text>
            <TouchableOpacity onPress={pickDocument} style={{ padding: 14, backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderRadius: 12, alignItems: 'center', borderStyle: 'dashed' }}>
              <Text style={{ color: form.resumeUrl ? COLORS.recruiterAccent : COLORS.textMuted, fontFamily: FONTS.bodySemiBold }}>
                {form.resumeUrl
                  ? (typeof form.resumeUrl === 'string' ? 'Document Attached ✓ (Tap to change)' : (form.resumeUrl.name || 'Document Attached (Tap to change)'))
        ...
        <View style={{ padding: 20, backgroundColor: COLORS.surface, borderTopWidth: 1, borderTopColor: COLORS.border }}>
          <Btn onPress={handleSave} variant="recruiter" fullWidth size="lg">
...
export default RecruiterEditProfileModal;
